<?php
session_start();
include '../src/connect_db.php';

if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $product_id = intval($_GET['id']);
} else {
    die("Invalid product ID.");
}

$sql = "SELECT * FROM products WHERE id = $product_id";
$result = $conn->query($sql);

if ($result->num_rows == 0) {
    die("Product not found.");
}

$product = $result->fetch_assoc();
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>
        <?php echo htmlspecialchars($product['name']); ?> - €<?php echo htmlspecialchars($product['price']); ?> 
    </title>
    <link href="../src/output.css" rel="stylesheet">
</head>
<body>
    <?php include '../public/components/navbar.php'; ?>

    <div class="max-w-screen-xl mx-auto mt-4 mb-40">
        <div class="flex flex-wrap lg:flex-nowrap lg:flex-row gap-4">
            <div class="w-full lg:max-w-2xl">
                <?php if (!empty($product['image_path'])): ?>
                    <img class="shadow-lg" src="<?php echo htmlspecialchars($product['image_path']); ?>" 
                        alt="<?php echo htmlspecialchars($product['name']); ?>">
                <?php else: ?>
                    <div class="w-full bg-gray-300 flex items-center justify-center">
                        <span class="text-gray-500">Nav attēla</span>
                    </div>
                <?php endif; ?>
            </div>

            <div class="px-6 w-full">
                <h1 class="text-4xl font-semibold text-gray-700"><?php echo htmlspecialchars($product['name']); ?></h1>
                <p class="text-primary"><?php echo htmlspecialchars($product['category'] ?? 'N/A'); ?></p>
                <div class="flex flex-wrap my-6 gap-4">
                    <h2 class="text-danger py-2 text-3xl font-bold">€<?php echo number_format($product['price'], 2); ?></h2>

                    <form method="POST" action="../src/addToCart.php">
                        <input type="hidden" name="product_id" value="<?php echo $product['id']; ?>">
                        <div class="flex items-center gap-2">
                            <input type="number" name="quantity" value="1" min="1" max="25" class="w-16 p-2 border rounded">
                            <button type="submit" class="py-3 px-3 bg-danger text-white font-semibold rounded-sm drop-shadow-sm 
                                   hover:drop-shadow-md hover:bg-opacity-90 active:scale-[1.01] flex flex-row gap-2">
                                <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M5.00014 14H18.1359C19.1487 14 19.6551 14 20.0582 13.8112C20.4134 13.6448 
                                             20.7118 13.3777 20.9163 13.0432C21.1485 12.6633 21.2044 12.16 21.3163 11.1534L21.9013 
                                             5.88835C21.9355 5.58088 21.9525 5.42715 21.9031 5.30816C21.8597 5.20366 21.7821 5.11697 
                                             21.683 5.06228C21.5702 5 21.4155 5 21.1062 5H4.50014M2 2H3.24844C3.51306 2 3.64537 2 3.74889 
                                             2.05032C3.84002 2.09463 3.91554 2.16557 3.96544 2.25376C4.02212 2.35394 4.03037 2.48599 
                                             4.04688 2.7501L4.95312 17.2499C4.96963 17.514 4.97788 17.6461 5.03456 17.7462C5.08446 17.8344 
                                             5.15998 17.9054 5.25111 17.9497C5.35463 18 5.48694 18 5.75156 18H19M7.5 21.5H7.51M16.5 
                                             21.5H16.51M8 21.5C8 21.7761 7.77614 22 7.5 22C7.22386 22 7 21.7761 7 21.5C7 21.2239 7.22386 
                                             21 7.5 21C7.77614 21 8 21.2239 8 21.5ZM17 21.5C17 21.7761 16.7761 22 16.5 22C16.2239 22 16 21.7761 
                                             16 21.5C16 21.2239 16.2239 21 16.5 21C16.7761 21 17 21.2239 17 21.5Z" 
                                      stroke="currentColor" 
                                      stroke-width="2" 
                                      stroke-linecap="round" 
                                      stroke-linejoin="round"
                                />
                                </svg>
                                Pievienot
                            </button>
                        </div>
                    </form>
                </div>
                
                <div class="relative overflow-x-auto mb-8">
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                <th scope="col" class="px-6 py-3">Materiāls</th>
                                <th scope="col" class="px-6 py-3">Dimensijas</th>
                                <th scope="col" class="px-6 py-3">Enerģijas Reitings</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td class="px-6 py-4"><?php echo htmlspecialchars($product['material']); ?></td>
                                <td class="px-6 py-4"><?php echo htmlspecialchars($product['dimensions']); ?></td>
                                <td class="px-6 py-4"><?php echo htmlspecialchars($product['energy_rating']); ?></td>
                            </tr>
                        </tbody>
                    </table>
                </div>

                <h2 class="text-xl font-bold text-gray-700">Apraksts</h2>
                <p><?php echo htmlspecialchars($product['description']); ?></p>
            </div>
        </div>
    </div>
    <?php include '../public/components/footer.php'; ?>
</body>
</html>
